...
class Jobs(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    address = db.Column(db.String(50), nullable=False) #'nullale=False' means they have to fill out address
...
def add_tech():
    ...
    form = TechnicianForm()
    if form.validate_on_submit():
        user = Technicians.query.filter_by(phone=form.phone.data).first() #DB Slot! if there is a 'first', this means that this user is already in db
        if user is None:
            user = Technicians(name=form.name.data, phone=form.phone.data, tech_rate=form.tech_rate.data) #DB Slot! defining new user to add to db
            db.session.add(user) #adding the user
            db.session.commit()  #comitting the addition
    ...
    return render_template("add_tech.html", form=form, name=name, our_users=our_users) #form, name, and our_users get passed into template
...
def update(id):
    ...
    if request.method == "POST":
        ...
        try:
            db.session.commit()
            flash("Technician updated sccessfully.")
